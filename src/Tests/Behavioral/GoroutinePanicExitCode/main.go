...
//
// GoFunc's exception filter only captures panic-convertible exceptions, and the goroutine root
// (golib.Goroutine) catches only GoexitException — a PanicException is deliberately left alone —
// so an unrecovered panic in a goroutine reaches golib's AppDomain.UnhandledException backstop.
// This test is what keeps that hole open: it fails the moment the root starts containing panics.
// That backstop used to print
