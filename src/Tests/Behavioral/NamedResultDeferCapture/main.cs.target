...
internal static (int64 written, error err) send(nint n) {
    ...
    ref var err = ref Ꮡerr.ValueSlot;
        ...
        defer(() => {
            fmt.Println((@string)"hook:"u8, written, Ꮡerr.ValueSlot, written > 0);
        });
        (var v, err) = pair(n);
        if (err != default!) {
...
internal static (int64, error) lit(nint n) {
    var f = () => {
        ...
        func((defer, recover) => {
            ...
            defer(() => {
                fmt.Println((@string)"lit hook:"u8, w, Ꮡe.ValueSlot);
            });
...
internal static error /*err*/ cls(nint n) {
    ...
    (var v, err) = pair(n);
    fmt.Println((@string)"cls pair:"u8, v, err);
    set();
    return err;
}
...
internal static void Main() {
    var (w, e) = send(3);
    fmt.Println((@string)"send:"u8, w, e);
    (w, e) = send(-1);
    fmt.Println((@string)"send:"u8, w, e);
    fmt.Println((@string)"addrv:"u8, addrv());
    (w, e) = lit(4);
