...
func (m *ModuleConverter) ConvertModule(moduleDir string) error {
	...
	//    converted import paths are, plus — under -recurse=nuget — the GoStdLibVersion default for the
	//    go.* NuGet PackageReferences.
...
//
// Under -recurse=nuget the props also defaults GoStdLibVersion — the version every emitted go.*
