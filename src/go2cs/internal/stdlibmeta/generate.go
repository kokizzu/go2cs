...
// sstringTwinPrefix is the sstring twins' record (sstringTwinOperations.go in the converter, which
// keys its reader on the same spelling): a NuGet-referenced package publishes its twins, so a consumer
// names a twin's canonical delegate rather than its method group, exactly as with an on-disk package.
const sstringTwinPrefix = "[assembly: GoSStringTwin("
...
func extract(packageInfoFile string) ([]string, error) {
	...
	for scanner.Scan() {
		...
		case strings.HasPrefix(trimmed, refPrimaryPrefix):
			lines = append(lines, trimmed)
		case strings.HasPrefix(trimmed, sstringTwinPrefix):
			lines = append(lines, trimmed)
		}
	}
