...
// seeds package_test_info.cs from that file: the RECOMPILE model, whose test assembly compiles the
// production `.cs` as its own items. Those files are NOT regenerated by a `-tests` run, so every
// alias they reference must carry over verbatim; production's binding is immovable and anything the
// test emission mints must yield to it.
//
// SESSION-scoped, not package-scoped: convertTestVariants fills it after the seed is written and
// before either variant converts, and convertTestVariant calls resetPackageState per variant — so
// clearing it there would empty it before the conversion that needs it. Cleared at the top of
// convertTestVariants instead, which is also what keeps a REFERENCE-model package (which seeds a
// test-class-only anchor and has no production aliases to honour) from inheriting the previous
// package's map. Same lifetime and the same reason as testAmbiguousLocalTypeNames.
var seededGlobalTypeAliases map[string]string

// qualifiedImportedTypeAliases marks the importedTypeAliases keys whose `global using` NAME is
// already bound by seededGlobalTypeAliases to a DIFFERENT target — two packages that share a base
// name and export the same type name, which go2cs's short-name-only alias key
// (`<rootPackageName>.<member>`, see importedTypeAliasSourceDirs) cannot tell apart.
//
// crypto/ecdh is the corpus's first: it imports crypto/internal/fips140/ecdh, whose package name is
// also `ecdh` and which also exports `PublicKey`, so production binds
// `ecdhꓸPublicKey = go.crypto.@internal.fips140.ecdh_package.ΔPublicKey` — while the EXTERNAL test
// package `ecdh_test` imports crypto/ecdh itself and would bind the same name to
// `go.crypto.ecdh_package.ΔPublicKey`. Under the recompile model both bindings land in ONE
// compilation: nist.cs and ecdh.cs reference the first, ecdh_test.cs the second, and C# has exactly
// one meaning per alias name (CS1537, crypto/ecdh's only build error).
//
// A marked key keeps its map entry — every existence check still sees it — but renders through the
// FULLY-QUALIFIED target instead of the alias name (getAliasedTypeName) and declares no
