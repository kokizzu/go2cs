...
public static void TestSub(ж<testing.T> Ꮡt) {
    var check = (@string desc, fs.FS subΔ1, error errΔ1) => {
        ...
        if (((sstring)data) != "goodbye, world"u8 || errΔ1 != default!) {
            Ꮡt.Errorf(@"ReadFile(%s, ""goodbye.txt"" = %q, %v, want %q, nil"u8, desc, ((@string)data), errΔ1, "goodbye, world");
        }
    ...
    if (err == default!) {
        Ꮡt.Fatal("Open(nonexist): succeeded");
    }
    ...
    if ((~pe).Path != "nonexist"u8) {
        Ꮡt.Fatalf("Open(nonexist): err.Path = %q, want %q"u8, (~pe).Path, "nonexist");
    }
    (_, err) = sub.Open("./"u8);
