...
internal static (image.Image, error) decodeFile(@string filename) => func<(image.Image, error)>((defer, recover) => {
...
public static void TestLargeImageWithShortData(ж<testing.T> Ꮡt) => func((defer, recover) => {
    ...
    var timer = time.AfterFunc(30000000000L, () => {
...
public static void TestExtraneousData(ж<testing.T> Ꮡt) {
    ...
    for ((nint i, nint nerr) = (0, 0); i < 1000 && nerr < 10; i++) {
        ...
        if (averageDelta(got, new jpeg_internal_test_package.image_ΔRGBAжImage(src)) > ((int64)2 << (int)(8))) {
