    ...
    sort.Interface
{
    ...
    any Pop();   // remove and return element Len() - 1.
}
...
public static void Push(Interface h, any x) {
    h.Push(x);
    up(h, h.Len() - 1);
}
...
public static any Pop(Interface h) {
    nint n = h.Len() - 1;
    h.Swap(0, n);
    down(h, 0, n);
    return h.Pop();
...
// The complexity is O(log n) where n = h.Len().
public static any Remove(Interface h, nint i) {
    nint n = h.Len() - 1;
    if (n != i) {
        h.Swap(i, n);
        if (!down(h, i, n)) {
...
// The complexity is O(log n) where n = h.Len().
public static void Fix(Interface h, nint i) {
    if (!down(h, i, h.Len())) {
        up(h, i);
    }
}
...
internal static bool down(Interface h, nint i0, nint n) {
    nint i = i0;
    while (ᐧ) {
        nint j1 = 2 * i + 1;
        if (j1 >= n || j1 < 0) {
