...
internal static ж<PrivateKey> testPrivateKey() {
    ...
    var (N, _) = bigmod.NewModulus(new byte[]{
        ...
        0x6B, 0x46, 0x01, 0x65, 0xC0, 0x8F, 0x53, 0x13}.slice());
    var (d, _) = bigmod.NewNat().SetBytes(new byte[]{
        ...
        0x97, 0x22, 0x3F, 0x7D, 0x8D, 0x0D, 0x43, 0x51}.slice(), N);
    var (p, _) = bigmod.NewModulus(new byte[]{
        ...
        0x47, 0x30, 0x45, 0xA5, 0x8E, 0x65, 0x53, 0x9D}.slice());
    var (q, _) = bigmod.NewModulus(new byte[]{
        ...
        0x93, 0x8D, 0x38, 0x68, 0x5D, 0x60, 0xBA, 0x6F}.slice());
    var (qInv, _) = bigmod.NewNat().SetBytes(new byte[]{
        ...
        0x2F, 0x67, 0xB5, 0xC8, 0xAA, 0x38, 0x7F, 0xF7}.slice(), p);
    var dP = new byte[]{
        ...
        0x8A, 0x27, 0xD9, 0xC8, 0xD0, 0x77, 0x2E, 0x65}.slice();
    var dQ = new byte[]{
        ...
        0xA7, 0x50, 0x6D, 0xEB, 0x52, 0x39, 0xA8, 0xA7}.slice();
...
internal static void initᴛfipsSelfTest() { fipsSelfTest = sync.OnceFunc(() => {
    fips140.CAST("RSASSA-PKCS-v1.5 2048-bit sign and verify"u8, error () => {
        ...
        var (sig, err) = signPKCS1v15(k, "SHA-256"u8, hash);
        if (err != default!) {
            return err;
        }
        {
            var errΔ1 = verifyPKCS1v15(k.PublicKey(), "SHA-256"u8, hash, sig); if (errΔ1 != default!) {
                return errΔ1;
            }
        }
