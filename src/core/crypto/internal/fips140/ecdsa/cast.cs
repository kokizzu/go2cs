...
internal static ж<PrivateKey> testPrivateKey() {
    ...
    return Ꮡ(new PrivateKey(
        pub: new ΔPublicKey(
            ...
            q: new byte[]{
                ...
                0xAB, 0x7E, 0x90, 0xA4, 0x2A, 0x49, 0x42, 0x72}.slice()
        ),
        d: new byte[]{
...
internal static slice<byte> testHash() {
    return new byte[]{
    ...
    where P : /* Point[P] */ new()
{
    fips140.PCT(ecdsaPctˢ, () => {
        ...
        return Verify(Ꮡc, Ꮡk.of(PrivateKey.Ꮡpub), hash, sig);
    });
}

internal static Action fipsSelfTest;
internal static void initᴛfipsSelfTest() { fipsSelfTest = sync.OnceFunc(() => {
    fips140.CAST("ECDSA P-256 SHA2-512 sign and verify"u8, error () => {
        var k = testPrivateKey();
        var Z = new byte[]{
            0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08,
            0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10
        }.slice();
        var persStr = new byte[]{
            0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18,
            0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20
        }.slice();
        var hash = testHash();
        var want = Ꮡ(new Signature(
            R: new byte[]{
...
internal static Action fipsSelfTestDeterministic;
internal static void initᴛfipsSelfTestDeterministic() { fipsSelfTestDeterministic = sync.OnceFunc(() => {
    fips140.CAST("DetECDSA P-256 SHA2-512 sign"u8, error () => {
        var k = testPrivateKey();
        var hash = testHash();
        var want = Ꮡ(new Signature(
            R: new byte[]{
