...
internal static uintptr wordSize => /* unsafe.Sizeof(uintptr(0)) */ 8;
...
internal static bool aligned(ж<byte> Ꮡdst, ж<byte> Ꮡx, ж<byte> Ꮡy) {
    return (uintptr)(((uintptr)((uintptr)((uintptr)Ꮡdst | (uintptr)Ꮡx) | (uintptr)Ꮡy)) & (uintptr)(wordSize - 1)) == 0;
}
...
{
    x = x[..(int)(len(dst))]; // remove bounds check in loop
    y = y[..(int)(len(dst))]; // remove bounds check in loop
