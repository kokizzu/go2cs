...
[GoRecv] internal static slice<byte> bytes(this ref P256Point p, [GoArrayDims(65)] ж<array<byte>> Ꮡout) {
...
[GoRecv] internal static (slice<byte>, error) bytesX(this ref P256Point p, [GoArrayDims(32)] ж<array<byte>> Ꮡout) {
    ref var @out = ref Ꮡout.DerefOrNull();
...
[GoRecv] internal static slice<byte> bytesCompressed(this ref P256Point p, [GoArrayDims(33)] ж<array<byte>> Ꮡout) {
    ref var @out = ref Ꮡout.DerefOrNull();
...
public static ж<P256Point> Add(this ж<P256Point> Ꮡq, ж<P256Point> Ꮡp1, ж<P256Point> Ꮡp2) {
    ...
    var t0 = @new<fiat.P256Element>().Mul(p1.x, p2.x); // t0 := X1 * X2
    var t1 = @new<fiat.P256Element>().Mul(p1.y, p2.y); // t1 := Y1 * Y2
    var t2 = @new<fiat.P256Element>().Mul(p1.z, p2.z); // t2 := Z1 * Z2
    var t3 = @new<fiat.P256Element>().Add(p1.x, p1.y); // t3 := X1 + Y1
    var t4 = @new<fiat.P256Element>().Add(p2.x, p2.y); // t4 := X2 + Y2
    t3.Mul(t3, t4); // t3 := t3 * t4
    t4.Add(t0, t1); // t4 := t0 + t1
    t3.Sub(t3, t4); // t3 := t3 - t4
    t4.Add(p1.y, p1.z); // t4 := Y1 + Z1
    var x3 = @new<fiat.P256Element>().Add(p2.y, p2.z); // X3 := Y2 + Z2
    t4.Mul(t4, x3); // t4 := t4 * X3
    x3.Add(t1, t2); // X3 := t1 + t2
    t4.Sub(t4, x3); // t4 := t4 - X3
    x3.Add(p1.x, p1.z); // X3 := X1 + Z1
    var y3 = @new<fiat.P256Element>().Add(p2.x, p2.z); // Y3 := X2 + Z2
...
public static ж<P256Point> Double(this ж<P256Point> Ꮡq, ж<P256Point> Ꮡp) {
    ...
    // Complete addition formula for a = -3 from "Complete addition formulas for
    // prime order elliptic curves" (https://eprint.iacr.org/2015/1060), §A.2.
    var t0 = @new<fiat.P256Element>().Square(p.x); // t0 := X ^ 2
    var t1 = @new<fiat.P256Element>().Square(p.y); // t1 := Y ^ 2
    var t2 = @new<fiat.P256Element>().Square(p.z); // t2 := Z ^ 2
    var t3 = @new<fiat.P256Element>().Mul(p.x, p.y); // t3 := X * Y
    t3.Add(t3, t3); // t3 := t3 + t3
    var z3 = @new<fiat.P256Element>().Mul(p.x, p.z); // Z3 := X * Z
