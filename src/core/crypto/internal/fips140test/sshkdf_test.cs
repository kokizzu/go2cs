// Copyright 2024 The Go Authors. All rights reserved.
// Use of this source code is governed by a BSD-style
// license that can be found in the LICENSE file.
namespace go.crypto.@internal;

using bytes = bytes_package;
using ssh = go.crypto.@internal.fips140.ssh_package;
using sha256 = go.crypto.sha256_package;
using hex = encoding.hex_package;
using testing = testing_package;
using encoding;
using go.crypto;
using go.crypto.@internal.fips140;
using hash = hash_package;
using Δfips140 = go.crypto.@internal.fips140_package;

partial class fipstest_internal_test_package {

public static void TestSSHACVPVector(ж<testing.T> Ꮡt) {
    // https://github.com/usnistgov/ACVP-Server/blob/3a7333f638/gen-val/json-files/kdf-components-ssh-1.0/prompt.json#L910-L915
    var K = fromHex(""u8);
    var H = fromHex("8FB22F0864960DA5679FD377248E41C2D0390E5AB3BB7955A3B6C588FB75B20D"u8);
    var sessionID = fromHex("269A512E7B560E13396E0F3F56BDA730E23EE122EE6D59C91C58FB07872BCCCC"u8);
    // https://github.com/usnistgov/ACVP-Server/blob/3a7333f638/gen-val/json-files/kdf-components-ssh-1.0/expectedResults.json#L1306-L1314
    var initialIVClient = fromHex("82321D9FE2ACD958D3F55F4D3FF5C79D"u8);
    var initialIVServer = fromHex("03F336F61311770BD5346B41E04CDB1F"u8);
    var encryptionKeyClient = fromHex("20E55008D0120C400F42E5D2E148AB75"u8);
    var encryptionKeyServer = fromHex("8BF4DEBEC96F4ADBBE5BB43828D56E6D"u8);
    var integrityKeyClient = fromHex("15F53BCCE2645D0AD1C539C09BF9054AA3A4B10B71E96B9E3A15672405341BB5"u8);
    var integrityKeyServer = fromHex("00BB773FD63AC7B7281A7B54C130CCAD363EE8928104E67CA5A3211EE3BBAB93"u8);
    var (gotIVClient, gotKeyClient, gotIntegrityClient) = ssh.Keys<Δfips140.Hash>(
        widen<hash.Hash, Δfips140.Hash>(sha256.New, elemᴛ0 => new fipstest_internal_test_package.hash_HashᴠHash(elemᴛ0)), ssh.ClientKeys, K, H, sessionID, 16, 16, 32);
    var (gotIVServer, gotKeyServer, gotIntegrityServer) = ssh.Keys<Δfips140.Hash>(
        widen<hash.Hash, Δfips140.Hash>(sha256.New, elemᴛ0 => new fipstest_internal_test_package.hash_HashᴠHash(elemᴛ0)), ssh.ServerKeys, K, H, sessionID, 16, 16, 32);
    if (!bytes.Equal(gotIVClient, initialIVClient)) {
        Ꮡt.Errorf("got IV client %x, want %x"u8, gotIVClient, initialIVClient);
    }
    if (!bytes.Equal(gotKeyClient, encryptionKeyClient)) {
        Ꮡt.Errorf("got key client %x, want %x"u8, gotKeyClient, encryptionKeyClient);
    }
    if (!bytes.Equal(gotIntegrityClient, integrityKeyClient)) {
        Ꮡt.Errorf("got integrity key client %x, want %x"u8, gotIntegrityClient, integrityKeyClient);
    }
    if (!bytes.Equal(gotIVServer, initialIVServer)) {
        Ꮡt.Errorf("got IV server %x, want %x"u8, gotIVServer, initialIVServer);
    }
    if (!bytes.Equal(gotKeyServer, encryptionKeyServer)) {
        Ꮡt.Errorf("got key server %x, want %x"u8, gotKeyServer, encryptionKeyServer);
    }
    if (!bytes.Equal(gotIntegrityServer, integrityKeyServer)) {
        Ꮡt.Errorf("got integrity key server %x, want %x"u8, gotIntegrityServer, integrityKeyServer);
    }
}

internal static slice<byte> fromHex(@string s) {
    var (b, err) = hex.DecodeString(s);
    if (err != default!) {
        throw panic(err);
    }
    return b;
}

} // end fipstest_internal_test_package
