...
public static void TestUSPacificZoneAbbr(ж<Δtesting.T> Ꮡt) => func((defer, recover) => {
    time_internal_test_package.ForceUSPacificFromTZIForTesting();
    // reset the Once to trigger the race
    defer(time_internal_test_package.ForceUSPacificForTesting);
    testZoneAbbr(Ꮡt);
});

public static void TestAusZoneAbbr(ж<Δtesting.T> Ꮡt) => func((defer, recover) => {
...
public static void TestToEnglishName(ж<Δtesting.T> Ꮡt) => func((defer, recover) => {
