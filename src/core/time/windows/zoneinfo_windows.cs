...
[assembly: go.GoPositionMap("time/zoneinfo_windows.go", "zoneinfo_windows.cs", "ABYyAA0CgoKUhKSCgpSSgIKkgIK4gpSClAAJCsKCgpSEgoKUgoKCptiSgoKCpqiSgoKClIKCgoK4lKwACAyCgoKClIKAgqaCgrbWooSEgoKUhISCgpSCgoKCgoKCzISCgoKogoKCgoKCqISCgoKCgoKChIKCggAfPIKCgIKCpA==")]
...
internal static int64 pseudoUnix(nint year, ref syscall.Systemtime d) {
    ...
    nint day = 1;
    var t = Date(year, ((ΔMonth)(nint)d.Month), day, (nint)d.Hour, (nint)d.Minute, (nint)d.Second, 0, ΔUTC);
    ...
    day += i;
    {
        nint week = (nint)d.Day - 1; if (week < 4){
            ...
            day += 4 * 7;
            if (day > daysIn(((ΔMonth)(nint)d.Month), year)) {
                day -= 7;
            }
        }
    }
...
internal static void initLocalFromTZI(ж<syscall.Timezoneinformation> Ꮡi) {
    ...
    if (nzone == 1) {
        // No daylight savings.
        std.Value.offset = -(nint)i.Bias * 60;
        l.Value.cacheStart = alpha;
        l.Value.cacheEnd = omega;
        l.Value.cacheZone = std;
        l.Value.tx = new slice<zoneTrans>(1);
        (~l).tx[0].when = l.Value.cacheStart;
        (~l).tx[0].index = 0;
        return;
    }
    ...
    // return above.
    std.Value.offset = -(nint)(i.Bias + i.StandardBias) * 60;
    var dst = Ꮡ((~l).zone, 1);
    dst.Value.name = dstname;
    dst.Value.offset = -(nint)(i.Bias + i.DaylightBias) * 60;
    dst.Value.isDST = true;
    // Arrange so that d0 is first transition date, d1 second,
    // i0 is index of zone after first transition, i1 second.
    var d0 = Ꮡi.of(syscall.Timezoneinformation.ᏑStandardDate);
    var d1 = Ꮡi.of(syscall.Timezoneinformation.ᏑDaylightDate);
    nint i0 = 0;
    nint i1 = 1;
    ...
    l.Value.tx = new slice<zoneTrans>(400);
    var t = Now().UTC();
    nint year = t.Year();
    nint txi = 0;
