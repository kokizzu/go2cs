...
[assembly: go.GoPositionMap("net/http/httptest/recorder.go", "recorder.cs", "AC9mkgAKHsKCgoKUAAYU8oKUgpaEgoKCgpSWqqKCgpSqooKClKYACxiCupKCloKCgoKUqqKClAAGJAAPAoKUgpTugoKUgoKUlISAgoKCooKUlIKClIKCyIKClIKUgqYAAhDSgoKUgoKU")]
...
[GoRecv] internal static void writeHeader(this ref ResponseRecorder rw, slice<byte> b, @string str) {
    ...
    var m = rw.Header();
    var (_, hasType) = m[contentTypeˢ, ꟷ];
    var hasTE = m.Get(transferEncodingˢ) != ""u8;
    if (!hasType && !hasTE) {
        if (b == default!) {
            b = slice<byte>(str);
        }
        m.Set(contentTypeˢ, http.DetectContentType(b));
    }
    rw.WriteHeader(200);
}
...
[GoRecv] public static ж<http.Response> Result(this ref ResponseRecorder rw) {
    ...
    res.Value.ContentLength = parseContentLength((~res).Header.Get(contentLengthˢ));
    {
        var (trailers, ok) = rw.snapHeader[trailerˢ, ꟷ]; if (ok) {
