...
[assembly: go.GoPositionMap("compress/flate/huffman_bit_writer_test.go", "huffman_bit_writer_test.cs", "")]
...
internal static ж<bool> update = flag.Bool("update"u8, false, "update reference files"u8);

// Hoisted @string literals (single allocation; Go keeps these in RODATA)
internal static readonly @string testdataHuffmanInˢ = "testdata/huffman-*.in"u8;
...
public static void TestBlockHuff(ж<testing.T> Ꮡt) {
    // determine input files
    var (match, err) = filepath.Glob(testdataHuffmanInˢ);
    if (err != default!) {
        Ꮡt.Fatal(err);
    }
...
// Hoisted @string literals (single allocation; Go keeps these in RODATA)
internal static readonly object outputOkˢ = (@string)"Output ok"u8;
internal static readonly object resetOkˢ = (@string)"Reset ok"u8;
...
internal static void testBlockHuff(ж<testing.T> Ꮡt, @string @in, @string @out) {
    ...
    bw.flush();
    var got = buf.Bytes();
    (var want, err) = os.ReadFile(@out);
    if (err != default! && !update.Value) {
        Ꮡt.Error(err);
        return;
    }
    Ꮡt.Logf("Testing %q"u8, @in);
    if (!bytes.Equal(got, want)) {
        if (update.Value) {
            if (@in != @out) {
                Ꮡt.Logf("Updating %q"u8, @out);
                {
                    var errΔ1 = os.WriteFile(@out, got, 438); if (errΔ1 != default!) {
                        Ꮡt.Error(errΔ1);
                    }
                }
                return;
            ...
            Ꮡt.Errorf("WARNING: -update did not rewrite input file %s"u8, @in);
        }
        Ꮡt.Errorf("%q != %q (see %q)"u8, @in, @out, @in + ".got");
        {
            var errΔ2 = os.WriteFile(@in + ".got"u8, got, 438); if (errΔ2 != default!) {
                Ꮡt.Error(errΔ2);
            }
        }
        return;
    ...
    got = buf.Bytes();
    if (!bytes.Equal(got, want)) {
        Ꮡt.Errorf("after reset %q != %q (see %q)"u8, @in, @out, @in + ".reset.got");
        {
            var errΔ3 = os.WriteFile(@in + ".reset.got"u8, got, 438); if (errΔ3 != default!) {
                Ꮡt.Error(errΔ3);
            }
        }
        return;
...
internal static void writeToType(ж<testing.T> Ꮡt, @string ttype, ж<global::go.compress.flate_package.huffmanBitWriter> Ꮡbw, slice<global::go.compress.flate_package.token> tok, slice<byte> input) {
    ...
    else if (exprᴛ1 == "dyn"u8) {
        bw.writeBlockDynamic(tok, false, input);
    }
    else { /* default: */
        throw panic("unknown test type");
    }
...
// Hoisted @string literals (single allocation; Go keeps these in RODATA)
internal static readonly object noOutputReceivedˢ = (@string)"no output received"u8;
internal static readonly object eofOkˢ = (@string)"EOF ok"u8;

// testWriterEOF tests if the written block contains an EOF marker.
internal static void testWriterEOF(ж<testing.T> Ꮡt, @string ttype, huffTest test, bool useInput) {
    if (useInput && test.input == ""u8) {
        return;
    }
    slice<byte> input = default!;
    if (useInput) {
        error err = default!;
    ...
    var b = buf.Bytes();
    if (len(b) == 0) {
        Ꮡt.Error(noOutputReceivedˢ);
        return;
    }
    if ((byte)(b[0] & 1) != 1) {
        Ꮡt.Errorf("block not marked with EOF for input %q"u8, test.input);
        return;
    }
    Ꮡt.Log(eofOkˢ);
}
