...
[assembly: go.GoPositionMap("internal/zstd/xxhash.go", "xxhash.cs", "ABw8opaChIKWgoSClKiShIKCgpaCgoKCgoKCloKCgoKCloKCupKCgpSIgoKCloSCgoKCgoKCgpSCgoKClIKCgoKWgoKCgoSokoKCgqiSgoKC")]
...
[GoRecv] internal static uint64 digest(this ref xxhash64 xh) {
    ...
    while (len >= 8) {
        ...
        buf = buf[8..];
        h64 ^= (uint64)(k1);
        h64 = bits.RotateLeft64(h64, 27) * (uint64)xxhPrime64c1 + (uint64)xxhPrime64c4;
        len -= 8;
    }
    if (len >= 4) {
        h64 ^= (uint64)((uint64)binary.LittleEndian.Uint32(buf) * (uint64)xxhPrime64c1);
        buf = buf[4..];
        h64 = bits.RotateLeft64(h64, 23) * (uint64)xxhPrime64c2 + (uint64)xxhPrime64c3;
        len -= 4;
    }
    while (len > 0) {
        h64 ^= (uint64)((uint64)buf[0] * (uint64)xxhPrime64c5);
        buf = buf[1..];
        h64 = bits.RotateLeft64(h64, 11) * (uint64)xxhPrime64c1;
        len--;
    }
