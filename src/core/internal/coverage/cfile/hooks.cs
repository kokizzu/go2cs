...
[assembly: go.GoPositionMap("internal/coverage/cfile/hooks.go", "hooks.cs", "AAw4ABYIgoKU")]
...
// terminates.
public static void InitHook(bool istest) {
    ...
    exithook.Add(new exithook.Hook(F: emitCounterData, RunOnFailure: true));
    if (istest){
        exithook.Add(new exithook.Hook(F: emitMetaData, RunOnFailure: true));
    } else {
        emitMetaData();
    }
}
