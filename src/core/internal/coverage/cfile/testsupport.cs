...
// generated code.
public static error ProcessCoverTestDir(@string dir, @string cfile, @string cm, @string cpkg, io.Writer w, slice<@string> selpkgs) {
    GoFrame ᒐ = default;
    try {
        ...
        if (tf != nil) {
            {
                var errΔ8 = (~ts).cf.EmitTextual(selpkgs, new os.FileжWriter(tf)); if (errΔ8 != default!) {
                    return errΔ8;
                }
            }
            tfClosed = true;
