...
[assembly: go.GoPositionMap("index/suffixarray/sais2.go", "sais2.cs", "")]
...
internal static void sais_8_64(slice<byte> text, nint textMax, slice<int64> sa, slice<int64> tmp) {
    ...
    nint numLMS = placeLMS_8_64(text, sa, freq, bucket);
    if (numLMS <= 1){
    } else {
        ...
        nint maxID = assignID_8_64(text, sa, numLMS);
        if (maxID < numLMS){
            map_64(sa, numLMS);
...
internal static void sais_32(slice<int32> text, nint textMax, slice<int32> sa, slice<int32> tmp) {
    ...
    // See the individual functions for documentation
    // about each's role in the algorithm.
    nint numLMS = placeLMS_32(text, sa, freq, bucket);
    if (numLMS <= 1){
    } else {
        // 0 or 1 items are already sorted. Do nothing.
        ...
        nint maxID = assignID_32(text, sa, numLMS);
        if (maxID < numLMS){
            map_32(sa, numLMS);
...
internal static void sais_64(slice<int64> text, nint textMax, slice<int64> sa, slice<int64> tmp) {
    ...
    // See the individual functions for documentation
    // about each's role in the algorithm.
    nint numLMS = placeLMS_64(text, sa, freq, bucket);
    if (numLMS <= 1){
    } else {
        // 0 or 1 items are already sorted. Do nothing.
        ...
        nint maxID = assignID_64(text, sa, numLMS);
        if (maxID < numLMS){
            map_64(sa, numLMS);
            recurse_64(sa, tmp, numLMS, maxID);
...
internal static slice<int64> freq_8_64(slice<byte> text, slice<int64> freq, slice<int64> bucket) {
...
internal static slice<int32> freq_32(slice<int32> text, slice<int32> freq, slice<int32> bucket) {
    if (freq != default! && freq[0] >= 0) {
        return freq; // already computed
    }
    if (freq == default!) {
...
internal static slice<int64> freq_64(slice<int64> text, slice<int64> freq, slice<int64> bucket) {
    if (freq != default! && freq[0] >= 0) {
        return freq; // already computed
    }
    if (freq == default!) {
...
internal static void bucketMin_8_64(slice<byte> text, slice<int64> freq, slice<int64> bucket) {
...
internal static void bucketMax_8_64(slice<byte> text, slice<int64> freq, slice<int64> bucket) {
    freq = freq_8_64(text, freq, bucket);
    freq = freq[..256]; // establish len(freq) = 256, so 0 ≤ i < 256 below
    bucket = bucket[..256]; // eliminate bounds check for bucket[i] below
    var total = (int64)0;
...
internal static void induceSubL_8_64(slice<byte> text, slice<int64> sa, slice<int64> freq, slice<int64> bucket) {
...
internal static void induceSubL_32(slice<int32> text, slice<int32> sa, slice<int32> freq, slice<int32> bucket) {
    // Initialize positions for left side of character buckets.
...
internal static void induceSubL_64(slice<int64> text, slice<int64> sa, slice<int64> freq, slice<int64> bucket) {
    // Initialize positions for left side of character buckets.
...
internal static void induceSubS_8_64(slice<byte> text, slice<int64> sa, slice<int64> freq, slice<int64> bucket) {
...
internal static void induceSubS_32(slice<int32> text, slice<int32> sa, slice<int32> freq, slice<int32> bucket) {
    // Initialize positions for right side of character buckets.
    bucketMax_32(text, freq, bucket);
...
internal static void induceSubS_64(slice<int64> text, slice<int64> sa, slice<int64> freq, slice<int64> bucket) {
    // Initialize positions for right side of character buckets.
    bucketMax_64(text, freq, bucket);
...
internal static void length_8_64(slice<byte> text, slice<int64> sa, nint numLMS) {
...
internal static void length_32(slice<int32> text, slice<int32> sa, nint numLMS) {
    nint end = 0; // index of current LMS-substring end (0 indicates final LMS-substring)
...
internal static void length_64(slice<int64> text, slice<int64> sa, nint numLMS) {
    nint end = 0; // index of current LMS-substring end (0 indicates final LMS-substring)
...
internal static void expand_8_64(slice<byte> text, slice<int64> freq, slice<int64> bucket, slice<int64> sa, nint numLMS) {
    bucketMax_8_64(text, freq, bucket);
...
internal static void expand_32(slice<int32> text, slice<int32> freq, slice<int32> bucket, slice<int32> sa, nint numLMS) {
    bucketMax_32(text, freq, bucket);
    // Loop backward through sa, always tracking
    // the next index to populate from sa[:numLMS].
    // When we get to one, populate it.
...
internal static void expand_64(slice<int64> text, slice<int64> freq, slice<int64> bucket, slice<int64> sa, nint numLMS) {
    bucketMax_64(text, freq, bucket);
    // Loop backward through sa, always tracking
    // the next index to populate from sa[:numLMS].
    // When we get to one, populate it.
...
internal static void induceL_8_64(slice<byte> text, slice<int64> sa, slice<int64> freq, slice<int64> bucket) {
    // Initialize positions for left side of character buckets.
    bucketMin_8_64(text, freq, bucket);
    bucket = bucket[..256]; // eliminate bounds check for bucket[cB] below
...
internal static void induceL_32(slice<int32> text, slice<int32> sa, slice<int32> freq, slice<int32> bucket) {
    // Initialize positions for left side of character buckets.
    bucketMin_32(text, freq, bucket);
...
internal static void induceL_64(slice<int64> text, slice<int64> sa, slice<int64> freq, slice<int64> bucket) {
    // Initialize positions for left side of character buckets.
    bucketMin_64(text, freq, bucket);
...
internal static void induceS_8_64(slice<byte> text, slice<int64> sa, slice<int64> freq, slice<int64> bucket) {
    // Initialize positions for right side of character buckets.
    bucketMax_8_64(text, freq, bucket);
    bucket = bucket[..256]; // eliminate bounds check for bucket[cB] below
    var cB = (byte)0;
