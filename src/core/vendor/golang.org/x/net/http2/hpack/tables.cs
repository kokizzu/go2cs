...
[assembly: go.GoPositionMap("vendor/golang.org/x/net/http2/hpack/tables.go", "tables.cs", "ACtagoKokqiSgoKCqJKClIKCgoKUgIK2goKUgoKUAAIgAA8CgoCCtoCCpKrCgpSCgpQ=")]
    ...
    if (!f.Sensitive) {
        {
            var id = t.byNameValue[new pairNameValue(f.Name, f.Value)]; if (id != 0) {
                return (Ꮡt.idToIndex(id), true);
            }
        }
    }
    {
        var id = t.byName[f.Name]; if (id != 0) {
            return (Ꮡt.idToIndex(id), false);
        }
    }
    return (0, false);
}
