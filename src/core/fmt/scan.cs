...
// Newlines in the input must match newlines in the format.
[OverloadResolutionPriority(1)] public static (nint n, error err) Sscanf(@string strʗp, sstring format, params ꓸꓸꓸany aʗp) {
    var a = aʗp.sslice();
...
[GoTwinForwarder] public static (nint n, error err) Sscanf(@string strʗp, @string format, params ꓸꓸꓸany aʗp) => Sscanf(strʗp, (sstring)format, aʗp);

// The canonical func value of Sscanf: a twinned function has no single method group (CS0123).
public static readonly Funcꓸꓸꓸ<@string, @string, any, (nint, error)> Sscanfᶠ = [GoTwinForwarder("Sscanf")] static (@string strʗp, @string format, ꓸꓸꓸany aʗp) => Sscanf(strʗp, format, aʗp);
...
// returns the number of items successfully parsed.
// Newlines in the input must match newlines in the format.
[OverloadResolutionPriority(1)] public static (nint n, error err) Fscanf(Δio.Reader r, sstring format, params ꓸꓸꓸany aʗp) {
    nint n = default!;
    error err = default!;
    var a = aʗp.slice();
...
[GoTwinForwarder] public static (nint n, error err) Fscanf(Δio.Reader r, @string format, params ꓸꓸꓸany aʗp) => Fscanf(r, (sstring)format, aʗp);

// The canonical func value of Fscanf: a twinned function has no single method group (CS0123).
public static readonly Funcꓸꓸꓸ<Δio.Reader, @string, any, (nint, error)> Fscanfᶠ = [GoTwinForwarder("Fscanf")] static (Δio.Reader r, @string format, ꓸꓸꓸany aʗp) => Fscanf(r, format, aʗp);
...
[GoRecv] internal static error UnreadRune(this ref readRune r) {
    if (r.peekRune >= 0) {
        return errors.New(fmtScanningCalledˢ);
    }
...
[GoRecv] internal static void SkipSpace(this ref ss s) {
    while (ᐧ) {
        ...
        if (r == (rune)'\n') {
            ...
            s.errorString(unexpectedNewlineˢ);
            return;
        }
        if (!isSpace(r)) {
            s.UnreadRune();
...
[OverloadResolutionPriority(1)] internal static nint indexRune(sstring s, rune r) {
...
[GoTwinForwarder] internal static nint indexRune(@string s, rune r) => indexRune((sstring)s, r);

// The canonical func value of indexRune: a twinned function has no single method group (CS0123).
internal static readonly Func<@string, rune, nint> indexRuneᶠ = [GoTwinForwarder("indexRune")] static (@string s, rune r) => indexRune(s, r);

// consume reads the next rune in the input and reports whether it is in the ok string.
// If accept is true, it puts the character into the input token.
[OverloadResolutionPriority(1)] [GoRecv] internal static bool consume(this ref ss s, sstring ok, bool accept) {
    var r = s.getRune();
    if (r == eof) {
        return false;
    }
...
[GoTwinForwarder] [GoRecv] internal static bool consume(this ref ss s, @string ok, bool accept) => s.consume((sstring)ok, accept);

// peek reports whether the next character is in the ok string, without consuming it.
[OverloadResolutionPriority(1)] [GoRecv] internal static bool peek(this ref ss s, sstring ok) {
    var r = s.getRune();
    if (r != eof) {
        s.UnreadRune();
    }
...
[GoTwinForwarder] [GoRecv] internal static bool peek(this ref ss s, @string ok) => s.peek((sstring)ok);
...
[OverloadResolutionPriority(1)] [GoRecv] internal static bool accept(this ref ss s, sstring ok) {
    return s.consume(ok, true);
}

[GoTwinForwarder] [GoRecv] internal static bool accept(this ref ss s, @string ok) => s.accept((sstring)ok);

// okVerb verifies that the verb is present in the list, setting s.err appropriately if not.
[OverloadResolutionPriority(1)] [GoRecv] internal static bool okVerb(this ref ss s, rune verb, sstring okVerbs, sstring typ) {
...
[GoTwinForwarder] [GoRecv] internal static bool okVerb(this ref ss s, rune verb, @string okVerbs, @string typ) => s.okVerb(verb, (sstring)okVerbs, (sstring)typ);
...
[GoRecv] internal static bool scanBool(this ref ss s, rune verb) {
    s.SkipSpace();
    s.notEOF();
    if (!s.okVerb(verb, "tv"u8, booleanˢ)) {
        return false;
    }
...
[OverloadResolutionPriority(1)] [GoRecv] internal static @string scanNumber(this ref ss s, sstring digits, bool haveDigits) {
    if (!haveDigits) {
        s.notEOF();
        if (!s.accept(digits)) {
            s.errorString(expectedIntegerˢ);
        }
    }
...
[GoTwinForwarder] [GoRecv] internal static @string scanNumber(this ref ss s, @string digits, bool haveDigits) => s.scanNumber((sstring)digits, haveDigits);
...
[OverloadResolutionPriority(1)] internal static bool hasX(sstring s) {
...
[GoTwinForwarder] internal static bool hasX(@string s) => hasX((sstring)s);

// The canonical func value of hasX: a twinned function has no single method group (CS0123).
internal static readonly Func<@string, bool> hasXᶠ = [GoTwinForwarder("hasX")] static (@string s) => hasX(s);
...
[GoRecv] internal static complex128 scanComplex(this ref ss s, rune verb, nint n) {
    if (!s.okVerb(verb, floatVerbs, complexˢ)) {
        return 0D;
    }
    s.SkipSpace();
    s.notEOF();
...
[GoRecv] internal static @string /*str*/ convertString(this ref ss s, rune verb) {
    ...
    if (!s.okVerb(verb, svqxXˢ, stringˢ2)) {
        return ""u8;
    }
    s.SkipSpace();
    s.notEOF();
...
[GoRecv] internal static @string quotedString(this ref ss s) {
    ...
    default: {
        s.errorString(expectedQuotedStringˢ);
        break;
    }}
...
[GoRecv] internal static (byte b, bool ok) hexByte(this ref ss s) {
    ...
    if (!ok) {
        s.errorString(illegalHexDigitˢ);
        return (b, ok);
    }
...
[GoRecv] internal static @string hexString(this ref ss s) {
    ...
    if (len(s.buf) == 0) {
        s.errorString(noHexDataForXStringˢ);
        return ""u8;
    }
    return ((@string)(slice<byte>)s.buf);
}
...
internal static void scanOne(this ж<ss> Ꮡs, rune verb, any arg) {
 ...
 floatVerbs, float32ˢ)) {
    ...
    case ж<float64> v: {
        if (s.okVerb(verb, floatVerbs, float64ˢ)) {
            s.SkipSpace();
            s.notEOF();
    ...
    try {
        ...
        if (s.nlIsEnd) {
            while (ᐧ) {
                ...
                if (!isSpace(r)) {
                    s.errorString(expectedNewlineˢ);
                    break;
                }
            }
        }
...
[OverloadResolutionPriority(1)] [GoRecv] internal static nint /*i*/ advance(this ref ss s, sstring format) {
    nint i = default!;
    ...
    while (i < len(format)) {
        ...
        if (isSpace(fmtc)) {
            ...
            for (nint j = 0; j < newlines; j++) {
                ...
                if (inputcΔ1 != (rune)'\n' && inputcΔ1 != eof) {
                    s.errorString(newlineInFormatDoesNotˢ);
                }
            }
            if (trailingSpace) {
                ...
                if (newlines == 0) {
                    ...
                    if (!isSpace(inputcΔ2) && inputcΔ2 != eof) {
                        s.errorString(expectedSpaceInInputToˢ);
                    }
                    if (inputcΔ2 == (rune)'\n') {
                        s.errorString(newlineInInputDoesNotˢ);
                    }
                }
...
[GoTwinForwarder] [GoRecv] internal static nint /*i*/ advance(this ref ss s, @string format) => s.advance((sstring)format);
...
[OverloadResolutionPriority(1)] internal static (nint numProcessed, error err) doScanf(this ж<ss> Ꮡs, sstring format, slice<any> a) {
    nint numProcessed = default!;
    heap<error>(out var Ꮡerr);
    GoFrame ᒐ = default;
    try {
        ...
        for (nint i = 0; i <= end; ) {
            ...
            if (format[i] != (rune)'%') {
                ...
                if (w < 0) {
                    s.errorString(inputDoesNotMatchFormatˢ);
                }
...
[GoTwinForwarder] internal static (nint numProcessed, error err) doScanf(this ж<ss> Ꮡs, @string format, slice<any> a) => Ꮡs.doScanf((sstring)format, a);
