...
[assembly: go.GoPositionMap("math/sin.go", "sin.cs", "AHzsAeKClKaCvJS4goSCkoKUgpaCgpSCloKClIKWgoKUlIKUAAIU8oKUpoK8lKS4goKCloKSgpSCloKClIKmgoKUgoKUlIKU")]
...
internal static float64 cos(float64 x) {
    ...
    if (j == 1 || j == 2){
        ...
        y = 1.0D - 0.5D * zz + zz * zz * ((((((_cos[0] * zz) + _cos[1]) * zz + _cos[2]) * zz + _cos[3]) * zz + _cos[4]) * zz + _cos[5]);
    }
    if (sign) {
        y = -y;
    }
