...
public static @string Text(this ж<Float> Ꮡx, byte format, nint prec) {
    ...
    return ((@string)Ꮡx.Append(new slice<byte>(0, cap), format, prec));
}
...
public static slice<byte> Append(this ж<Float> Ꮡx, slice<byte> buf, byte fmt, nint prec) {
    ...
    if (x.form == inf) {
        ...
        return append(buf, ((@string)"Inf"u8).ꓸꓸꓸ);
    }
    ...
    case (rune)'g' or (rune)'G': {
        ...
        if (exp < -4 || exp >= eprec) {
            ...
            return fmtE(buf, (byte)(fmt + (rune)'e' - (rune)'g'), prec - 1, d);
        }
        if (prec > d.exp) {
            prec = len(d.mant);
        }
...
internal static void roundShortest(ж<@decimal> Ꮡd, ж<Float> Ꮡx) {
    ...
    ref var lower = ref heap(new @decimal(), out var Ꮡlower);
    nat tmp = default!;
    Ꮡlower.init(tmp.sub(mant, natOne), exp);
    // 3) Compute upper bound by adding 1/2 ulp.
    ref var upper = ref heap(new @decimal(), out var Ꮡupper);
    Ꮡupper.init(tmp.add(mant, natOne), exp);
...
[GoRecv] internal static slice<byte> fmtB(this ref Float x, slice<byte> buf) {
    ...
    var m = x.mant;
    {
        var w = (uint32)len(x.mant) * (uint32)_W;
        switch (ᐧ) {
        case {} when w < x.prec: {
...
internal static slice<byte> fmtX(this ж<Float> Ꮡx, slice<byte> buf, nint prec) {
    ref var x = ref Ꮡx.DerefOrNull();

    if (x.form == zero) {
        buf = append(buf, ((@string)"0x0"u8).ꓸꓸꓸ);
        if (prec > 0) {
            buf = append(buf, (byte)((rune)'.'));
            for (nint i = 0; i < prec; i++) {
                buf = append(buf, (byte)((rune)'0'));
        ...
        buf = append(buf, ((@string)"p+00"u8).ꓸꓸꓸ);
        return buf;
    }
    if (debugFloat && x.form != finite) {
        throw panic("non-finite float");
    ...
    // n%4 == 1
    Ꮡx = @new<Float>().SetPrec(n).SetMode(x.mode).Set(Ꮡx); x = ref Ꮡx.DerefOrNull();
    // adjust mantissa to use exactly n bits
    var m = x.mant;
    {
        nuint w = (nuint)len(x.mant) * (nuint)_W;
        switch (ᐧ) {
        case {} when w < n: {
...
internal static fmt.Formatter _ᴛ3ʗ = new FloatжFormatter(ᏑfloatZero);   // *Float must implement fmt.Formatter
...
// for details.
public static void Format(this ж<Float> Ꮡx, fmt.State s, rune format) {
    ref var x = ref Ꮡx.DerefOrNull();
