...
internal static suspendGState suspendG(ж<g> Ꮡgp) {
    ...
    var stopped = false;
    ж<m> asyncM = default!;
    uint32 asyncGen = default!;
    int64 nextPreemptM = default!;
    for (nint i = 0; ᐧ ; i++) {
        {
...
internal static partial void asyncPreempt();
...
/* [GoInit] runtime bootstrap init - not run; .NET is the runtime */ internal static void initΔ5() {
    ...
    if (asyncPreemptStack > stackNosplit) {
        ...
        print((@string)"runtime: asyncPreemptStack="u8, asyncPreemptStack, (@string)"\n"u8);
...
internal static (bool, uintptr) isAsyncSafePoint(ж<g> Ꮡgp, uintptr pc, uintptr sp, uintptr lr) {
    ...
    if ((GOARCH == "mips"u8 || GOARCH == "mipsle"u8 || GOARCH == "mips64"u8 || GOARCH == "mips64le"u8) && lr == pc + 8 && funcspdelta(f, pc) == 0) {
    ...
    var (up, startpc) = pcdatavalue2(f, abi.PCDATA_UnsafePoint, pc);
    ...
    {
        @unsafe.Pointer fd = (uintptr)funcdata(f, abi.FUNCDATA_LocalsPointerMaps); if (fd == nil || (abi.FuncFlag)(f.flag & abi.FuncFlagAsm) != 0) {
