...
namespace BehavioralRunner
{
    ...
    internal static class Runner
    {
        ...
        // every machine and every future corpus size.
        //
        ...
        // budget.
        //
        // The defaults below are UNCHANGED from those constants, deliberately: the fast-desktop lane
        // keeps its existing fail-fast behavior, and only an explicit opt-in (flag or environment
        // variable, resolved in Main) raises a budget. Raising them unconditionally would trade one
        // machine's false red for every machine's slow hang.
        private const int DefaultBuildAllTimeoutMs = 300_000;  // one-shot parallel build of every C# target
        private const int DefaultBuildOneTimeoutMs = 180_000;  // per-project fallback build / go build
