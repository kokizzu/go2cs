# `crypto/internal/fips140/ecdsa` — validation proof

Go's own `crypto/internal/fips140/ecdsa` test suite, converted to C# by [go2cs](https://github.com/ritchiecarroll/go2cs), built against the converted standard
library, run under the Go-semantics test host, and compared verdict for verdict against a clean
`go test -json` baseline of the same sources. This page is generated by the converter from that
comparison — it is the evidence behind the `crypto/internal/fips140/ecdsa` row in
[Validated Test Packages](ValidatedTestPackages.md).

*Validated 2026-09-22 · converter `c6fdbe73c`*

**10 matched · 0 disclosed** — Go 1.24.13, `windows/amd64`, converted package
[`src/core/crypto/internal/fips140/ecdsa`](https://github.com/ritchiecarroll/go2cs/tree/nuget-1.24.13.1/src/core/crypto/internal/fips140/ecdsa).

Measured at `Release` (tiered JIT off), oracle `go version go1.24.13 windows/amd64`.

## Verdicts

| Test | `go test` | go2cs |
|:--|:--:|:--:|
| `TestHashToNat` | pass | pass |
| `TestHashToNat/P-224` | pass | pass |
| `TestHashToNat/P-256` | pass | pass |
| `TestHashToNat/P-384` | pass | pass |
| `TestHashToNat/P-521` | pass | pass |
| `TestRandomPoint` | pass | pass |
| `TestRandomPoint/P-224` | pass | pass |
| `TestRandomPoint/P-256` | pass | pass |
| `TestRandomPoint/P-384` | pass | pass |
| `TestRandomPoint/P-521` | pass | pass |
