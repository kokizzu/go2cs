...
See [Named Numeric Types and Constant Contexts](#named-numeric-types-and-constant-contexts) for how these interact with native-int and named numeric types. See also [example](https://github.com/ritchiecarroll/go2cs/tree/master/src/Examples/Manual%20Tour%20of%20Go%20Conversions/basics/numeric-constants).
...
### A black-box-only suite REFERENCES the production project instead of recompiling it

The original `-tests` model — recompile the production `.cs` into the test assembly — is what lets test files bind unexported production symbols, but it **duplicates the production types**: the test assembly's recompiled `unicode.SpecialCase` is a *different type* from the one inside the referenced production assembly. That is harmless until some *other* referenced assembly surfaces a production type in its public API — `strings.ToLowerSpecial(unicode.SpecialCase, …)` names the type in the **production** `unicode` assembly, which the test project does not reference — CS0012 (unicode's `letter_test`, blocked for two waves). No compile-set arrangement fixes this inside one assembly; the type identity itself is the problem.

`-tests` therefore selects between **two test-project models** (`selectTestProjectModel`, recorded as `testProjectModel` in the manifest):

- **`recompile`** (the original model) — production sources are compile items of the test assembly; `package_test_info.cs` is seeded verbatim from the production `package_info.cs` (production class + records local); the self-import binds to the local partial class. Required whenever an **internal** (in-package) test variant exists, because its files reach unexported production symbols only a same-assembly compile can bind.
- **`reference`** — used when the suite is **black-box only** (gated strictly on `internal == nil` in the loaded variant model): the test project carries a **colocated `<ProjectReference Include="<name>.csproj" />`** and drops every production compile item, so the production assembly stays the *single identity* for its types. The production package binds as an **ordinary imported package** — the `-tests` self-import rebinding (`testPackagePath`/`testPackageName`, `testLocalTypePrefixes`, the production-implements seed) is simply not engaged, so its exported aliases load from the colocated `package_info.cs` like any dependency's, its types render package-qualified, and `isSameAssemblyPkg` answers false, composing the same foreign adapter names go2cs-gen generates for any project-referenced package. `package_test_info.cs` is seeded as a **test-class-only anchor** (`referenceModelTestPackageInfoSeed`): its first — and only — class is `<name>_test_package` carrying `[GoPackage]` directly, and **no production class partial is declared** — a local shadow declaration (or any generated code anchored to one) would re-introduce exactly the duplicate-type problem the model eliminates. There is no `package_info_external_test.cs` under this model: one anchor, one file.

**Fallback:** a reference-model conversion that collects records demanding a **production anchor** — a `GoImplement` partial/ж-adapter that must merge into a production type declaration, `GoImplicitConv` operators on a production type, or any record rendering a production type through its imported `ꓸ` alias form (`recordsRequireProductionAnchor`, evaluated with the same partition predicates as the two-anchor split) — cannot host that generated code across the assembly boundary, and the conversion **re-runs under the recompile model** (`errProductionAnchoredRecords`; deterministic — the `go/packages` load is shared, the fallback costs one extra emission pass). Test-anchored records (bare test-local implementers, adapter-class ᴠ pairs) do *not* trigger the fallback — they generate in the test class and work cross-assembly.
...
* **GOARCH-gated with no `purego` escape** — the platform file is gated only on architecture (`amd64 || arm64`) with no `|| purego` alternative, so the tag cannot reach a pure-Go variant (`internal/chacha8rand`'s `chacha8_amd64.s`; `hash/crc32`'s `crc32_amd64.go`, which carries no `//go:build` line at all — its only constraint is the `_amd64` filename suffix, so `go list -tags purego hash/crc32` selects exactly the same files as the default build). These are hand-owned: either as a `<name>_impl.cs` companion or as a whole-file rewrite (the mechanisms in *Manually-Converted Declarations* below). Hand-owning does **not** have to mean stubbing — where .NET exposes the same instructions the `.s` file issues, the arch layer can be realized for real; see *[Realizing an asm-backed arch layer with managed hardware intrinsics](#realizing-an-asm-backed-arch-layer-with-managed-hardware-intrinsics)*.
...
Note the csproj constraint: the converter **regenerates each package's `.csproj` on every transpile**, and it sets `AllowUnsafeBlocks=false`. A hand-owned file therefore cannot use `byte*`/`fixed`/`stackalloc` — any setting added by hand would be clobbered on the next run. The loads go through `MemoryMarshal.GetReference(p.ToSpan())` plus `Unsafe.ReadUnaligned<T>` / `Unsafe.Add`, which need no compiler flag and read the slice's real backing window (offset included) with no copy.
