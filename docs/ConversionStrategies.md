...
contract itself, walking a snapshot of the keys and re-reading each value on arrival. The emitted
...
long: it is what hung `net/http`'s HTTP/2 server in `promoteUndeclaredTrailers`. See the
