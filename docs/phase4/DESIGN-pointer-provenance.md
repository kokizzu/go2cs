...
**STATUS: RATIFIED (coordinator, 2026-08-23). ⟨OQ-P1⟩ the table, ⟨OQ-P3⟩ AMENDED on the mode owner’s concurrence (G, mailbox, 2026-08-23): ONE increment, TWO GATED HALVES — a slice-only patch would invent a throwaway local provenance signal, and the two are one defect wearing two hats; R lands the MECHANISM first with standalone gates (table + registration + GolibTests), then G lands the slice CONSUMER on top (the guard gains a provenance test; the native ctor refuses a pinned-managed address or retains its pin; the falsified `unsafe.cs:653` "lifetime is the mapping’s own" comment amended whichever way). Gate #2 is CLOSED-FORM per G: `OverNativeMemory` has one caller corpus-wide, so the audit is "which of the 52 non-test `unsafe.Slice` sites can supply a pinned-managed pointer" — G runs it as the mode’s owner, ⟨OQ-P4⟩ census-first chooses — all per recommendation. **⟨OQ-P2⟩ RATIFIED WITH A REFINEMENT that closes the ABA window structurally**: weak entries dying with their box (your CWT tie), OVERWRITE-on-register (the latest pin owns the address, so same-type reuse is benign — every pin re-registers), and read = the recovered box is ALIVE and its CURRENT pinned address equals the queried one, else MISS. With validate-on-read in place the (address, type) key adds nothing — key by address alone. The residual window closes on liveness itself: a live box whose pinned address still matches genuinely occupies that storage, so no native allocation can coexist there. Gate #2 (the slice liveness audit) and gate #1 (the census, already probing) precede the mechanism as §5 orders-view SAFETY FLOOR was
