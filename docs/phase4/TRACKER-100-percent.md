...
| In flight | G (only lane armed): CNR at the A3 merged head → merge-shape verdict, then the registry both-sides guard (+ dead-entry cleanup riding it), then the crypto/tls third-host-state sweep encoding; R (arming): InterfaceData two halves per the parked spec, then the tail; i9 (arming): PageCache cast, then residuals to ZERO → the SEMANTIC BILL, traceback pre-pass ruled theirs (Stage-A, sizing-first); coordinator: CLAUDE.md doctrine batch, then wave Stage A completion per `docs/PLAN-rebank-wave.md` |
...
4. CLAUDE.md doctrine batch (eight accumulated instrument items) — coordinator, in progress
