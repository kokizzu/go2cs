...
(r58b, 2026-08-09).** Unlike `MapRange`/`MapKeys`, `MapIndex` was never hand-converted: it reads
`v.ptr` as flat memory and calls `mapaccess`, so it faults on any Value the managed bridge produced
(a nil-pointer-dereference panic, not a wrong answer). Nothing in the validated roster calls it yet,
which is why it has stayed invisible; `encoding/json` and any reflective map walk will reach it.
Small and well-shaped — the `MapRange` iterator already has the machinery (`iter.mapValueType` →
`makeTypedValue`), so `MapIndex` is a lookup plus the same slot-typed construction.
