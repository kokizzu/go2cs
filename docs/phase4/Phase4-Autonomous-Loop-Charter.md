...
| **math/big** (Tier-0 #4) | 189 Test funcs; its own campaign. **PREREQUISITE (user, 2026-07-31): rename the `GoUntyped` C# alias first — DONE 2026-08-04 (r40-rebank commit A).** The alias (= `System.Numerics.BigInteger`, `golib.csproj` `<Using Alias>`) was the ORIGINAL name from before golib's untyped numeric wrapper structs (`UntypedInt`/`UntypedFloat`/…) existed, and had come to read as if it were one of them; it is actually the arbitrary-precision carrier for untyped constants exceeding native width. **Name RULED (user, 2026-08-02): `GoBigConst`** — it says exactly what the alias is (the arbitrary-precision carrier for oversized untyped constants) and cannot be confused with the `UntypedInt`/`UntypedFloat` wrappers or math/big's own `big.Int` mapping. The rename RODE THE WHOLE-CORPUS REBANK: pure rename in commit A (converter emission + both csproj templates + `golib.csproj` + goldens + docs), corpus levelled by the rebank's own regen in commit B. Note the arc's ground floor now exists: math/big's arithmetic WORKS as of 2026-08-02 (`math_big_pure_go` tag fix) — first newly-reachable defect (`big.Int.GCD` nil-deref → all of `big.Rat`) is board-recorded. Footprint measured 2026-07-31: converter emits it from 5+ `.go` files; **685 corpus files** reference it → converter + golib + one deliberate whole-corpus regen, gated as usual. Doing it BEFORE math/big avoids renaming under a much larger `BigInteger` surface. | After the reflection remainder lands (depends on it through gob/json/xml + testing/quick). |
