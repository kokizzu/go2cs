...
17. One type per file, named after the type — that is the default and it stays the default. Split a type across `partial` files only when a large, *mechanically shaped* cluster of members (an arity ladder, a generated overload set) is crowding out the members a reader actually comes to the file to find. Name each additional file `<Type>.<Cluster>.cs` (`builtin.GoroutineLaunchers.cs`, `builtin.DeferRegistrations.cs`), and open it with a banner comment stating what the cluster covers, *why* the members take that shape, and the rule for adding to it — so the next contributor can tell at a glance whether their new member belongs there or in the primary file. Do not split a type whose size is cohesive logic rather than repetition; that is a design problem, not a file-layout one.
