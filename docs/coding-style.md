...
18. Never name a local, parameter, pattern variable or range variable `field` inside a property accessor — including inside a local function or lambda nested in one. C# 14 makes `field` a *keyword* in that scope (it names the property's synthesized backing field), so the declaration is CS9273 and every use rebinds away from your variable. Every hand-authored project here sets `LangVersion` to `latest`, so a name that compiles fine on the SDK you happen to have becomes a hard build break on the next one — that is exactly how `go2cs-gen` stopped building for anyone on the .NET 10 SDK ([issue #34](https://github.com/ritchiecarroll/go2cs/issues/34)), and an older SDK will not warn you. Use a qualified name (`fieldSymbol`, `structField`); the `@field` escape compiles but reads as a workaround. Apply the same care to any future contextual keyword. This rule governs hand-authored code only — the escapes that keep *generated* identifiers legal (`file`, `required`, `scoped`, …) live in the converter's keyword set, `src/go2cs/identifierNaming.go`.
