...
| [`math/big`](https://github.com/ritchiecarroll/go2cs/tree/master/src/core/math/big) | 230 | 1 | Arbitrary-precision `Int`/`Float`/`Rat` arithmetic — the karatsuba/toom multiplication thresholds, GCD and modular machinery, string/scan round-trips across bases and formats; two alloc-profile disclosures, one on the `MemStats` byte meter (the unbalanced-multiply temp-`nat` workspaces the ж-box arc's math/big harvest reclaims). · linux: 230 + 1 · [proof](validation/current/math.big.md) |
