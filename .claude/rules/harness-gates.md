...
- **A STANDALONE (no-solution-context) build of anything under `src/tests` measures the machine-global DEPLOY ROOT, not the repo — and its errors look SEMANTIC.** Without `$(SolutionDir)`, `$(go2csPath)` falls back to `%USERPROFILE%/go2cs` / `%GOPATH%\src\go2cs`, which is STALE between deploys. A *missing* root is loud (CS0246 on `go`); a *stale* one produces plausible type-mismatch errors (CS1503, CS1929, CS0234 on a newer attribute) that read as real regressions and are COMMIT-INDEPENDENT. **Pin `-p:go2csPath=<repo>/src/` (forward slashes) on every standalone build under `src/tests`, and a bisect probe must carry the pin.** **The same holds for a SOLUTION build of `go2cs.slnx` in any non-Debug configuration:** every behavioral csproj sets `go2csPath` to `$(USERPROFILE)/go2cs/` when `$(Configuration)` is not `Debug` and only uses `$(SolutionDir)` under Debug, so `dotnet build src/go2cs.slnx -c Release` reads the deploy root even inside the solution. Pin `-p:go2csPath=<repo>/src/` on it too. <!-- R-B16, ruled at the H10 close seat's STOP 3 (ledger 2026-09-23 21:43, mailbox 00133ee238; i7, B9(f) at 1a328f3ee8): the brief's unpinned `dotnet build src/go2cs.slnx -c Release` read 15,240 CS0246/CS0234 across 695 behavioral projects on a box with no deploy root; the same build with -p:go2csPath=<repo>/src/ was rc 0, 0 errors, 925 assemblies, and the PINNED arm is the reading of record because it builds THIS tree. Measured at fa18863b94: 698 of the 699 behavioral csprojs carry the Configuration-keyed fallback (e.g. src/tests/Behavioral/AdapterNameInterfaceCollision/AdapterNameInterfaceCollision.csproj:41-43). --> <!-- Paid 2026-08-30, cost one full invalidated bisect: a bisect probe built standalone reported
...
- **For a pure converter no-regression check with no compile/run at all, use `check-no-regression.ps1`** — it re-transpiles every behavioral dir and `git status`es the converter-emitted **`.cs` AND `.csproj`** (the transpile rewrites both; the `.cs`-only pathspec it had until 2026-08-08 made a csproj-emission change invisible on every platform). Converter stderr is captured, not discarded: a package the run could not fully regenerate — best-effort "did not fully type-check", a recovered "visit file error", or a non-zero exit — **fails the gate BY NAME as `NOT MEASURED` even with a clean `git status`**, so the byte-identical verdict is never vacuous; other WARNINGs are advisory, never fatal. <!-- Coordinator ruling 2026-08-08, from lane r48b's Linux FindFirstFileData finding — see
  ...
  - CNR refuses with exit 2 when `git` or `go` cannot be resolved; before that refusal a `git` missing from PATH printed "not recognized" and exited 0, so a CNR green from a tree without the refusal is not a reading. <!-- 2026-09-26 routing (C1, hop lessons appendix D): 2026-09-22: check-no-regression.ps1 exited 0 with git absent from PATH (CNR_RC=0), a FALSE GREEN; the refusal landed on the version tip. ledger 2026-09-22 09:46 STAMP 598d1978a; d095fe8108 [L#59] -->
...
- **A COMMAND THAT DOES TWO THINGS CAN HALF-SUCCEED, AND THE EXIT CODE TELLS YOU NOTHING ABOUT WHICH HALF — a timeout does not roll back what already ran.** Check BOTH effects independently. **And a tool's own timeout is not the COMMAND's timeout; the shorter one wins silently** — a solution build given a 30-minute internal budget died at 10, the harness cap, reporting exit 143 with an empty log, which reads exactly like a build that produced nothing. <!-- 2026-09-07: one invocation launched a detached build, wrote a mailbox entry file and posted it;
  ...
  - Size an OUTER budget from the instrument's WORST case: `run-validated-sweep.ps1` can take two attempts (the oracle re-run) × two test sides per row, so a lane driver's per-row budget is `4 × max(ask, floor) + 30 min`; on a breach, read the results tail first, then kill only its own child tree by recorded PID. <!-- 2026-09-26 routing (C1, hop lessons appendix D): 2026-09-24: repair-log defect M5, a 3x + 15 min wrapper budget was itself a false-red generator. briefs/full-roster-sweep-repair-log.md:29 [R#33] -->
...
- **A SHARED PowerShell instrument owes a run on BOTH editions before it banks: 5.1 on a Windows lane AND 7 on a Linux lane — or the OS-matrix linux leg — before the change merges.** The check IS the PARSE of every shared script under pwsh 7 Core, plus one row actually run. **Measured is not parsed, and not-yet-closed is not untested — say which of the three a check was.** <!-- Measured 2026-09-02: _roster.ps1's comparison reader took `Add-Type -AssemblyName
  ...
  <!-- 2026-09-26 routing (C1, hop lessons appendix D): 2026-09-24: repair-log defect m11 confirmed PS 5.1 *> writes UTF-16LE; check-roster-format.ps1's ledger-scope self-test fails under 5.1 even on master and passes under pwsh 7 (obligation d22, HOST-DEPENDENT). briefs/full-roster-sweep-repair-log.md:49 [R#13] -->
