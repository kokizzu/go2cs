...
| **Behavioral tests** | `src/tests/Behavioral/` (547 test projects = 572 transpiled Go packages incl. 25 nested sub-libraries; 574 registered `.csproj` incl. the `BehavioralTests`/`BehavioralRunner` tooling — re-measured 2026-08-07 at r43g: r41's DeferFrameScopes, r41c's RangeOverIntegerTypes and r43g's RuntimeCallerFrames joined) | Go + C# | Per-feature Go↔C# equivalence (arrays, channels, defer, generics, interfaces…). |
