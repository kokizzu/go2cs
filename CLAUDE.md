...
| **Standard library** | `src/core/<pkg>` | C# (converted) | The whole Go stdlib (302 packages, Go 1.23.1) auto-converted by `go2cs -stdlib`. **Compiles clean** (Phase-3 milestone, 2026-07-10); 69 packages **validate operationally** against `go test`. Two packages are hand-owned and never queued for conversion: `unsafe` and `testing` (the Phase-4 test host). Its own `src/go2cs-stdlib.slnx`, generated by the converter and adopted verbatim. |
| **Behavioral tests** | `src/Tests/Behavioral/` (528 test projects = 553 transpiled Go packages incl. 25 nested sub-libraries; 555 registered `.csproj` incl. the `BehavioralTests`/`BehavioralRunner` tooling) | Go + C# | Per-feature Go↔C# equivalence (arrays, channels, defer, generics, interfaces…). |
...
- **Behavioral tests** (`src/Tests/Behavioral/`): each test references `golib` + the `go2cs-gen` analyzer;
  ...
  `reflect`). Since 2026-08-01 those references bind the **converted** packages, so the suite's ~488
...
- **FALSE-GREEN route #3 — NESTED sub-library packages were never enumerated (fixed 2026-08-02).** All
  ...
  **recursively, DEEPEST-FIRST** (`GoPackageDirs` in `BehavioralRunner`/`BehavioralTestBase`; a recursive
  ...
  `UpToDate` in both runners considers the nested packages too. Enumeration is now **553 packages**
  (528 top-level + 25 nested; it was 543 = 521 + 22 when this note was written), not 521. Note goldens remain top-level-only: nested packages have no
...
- **Budget each command against its MEASURED baseline — the old flat "~3 min" cap is no longer right for
  the full runs (re-measured 2026-08-02, corpus at 553 transpiled packages / 555 registered `.csproj`).** The
  corpus keeps growing (371 → 457 → 518 → 543 packages), and both full instruments
  ...
  |---|---|---|---|
  | `run-behavioral.ps1` (full, 4 phases) | **~370–1575s (6–26 min; 533s measured 2026-08-02, 1575s the same day with THREE sibling worktrees running pipelines)** | 2100s | 528/528 Transpile+Compile+Target; 497 Output-compared, 31 skipped (no `package main`); the top of the range is concurrent-lane load — budget for it. ⚠ At that load the **Go toolchain itself** can crash building one project (`panic: … compress/flate.(*huff…` inside `go build`) and the runner reports it as a Go build failure; re-run that one project filtered before believing it |
  | `check-no-regression.ps1` (full) | **~350–475s (6–8 min; re-measured 2026-08-02 at **543 packages** — 365s — after the walk went recursive, so the +22 nested packages cost nothing detectable; top of range is concurrent-lane load)** | 700s | transpile-only, no compile/run; re-transpiles unconditionally |
  ...
  - **⚠ After an operational SWEEP, `git status` is dirty and it is (almost always) NOTHING — classify,
    ...
    1. **CRLF phantoms — the whole of a healthy sweep's dirt.** The converter preserves the Go source's
       ...
       **modified with an EMPTY content diff**. **The counting rule: `git diff --numstat` must be empty.**
       If it is, `git checkout -- src/core` and move on. Do **not** memorize a file list —
