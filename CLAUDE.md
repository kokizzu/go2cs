...
| **Behavioral tests** | `src/tests/Behavioral/` (549 top-level test projects = 574 transpiled Go packages incl. 25 nested sub-libraries; 576 registered `.csproj` incl. the `BehavioralTests`/`BehavioralRunner` tooling — re-measured 2026-08-07 at r44b, which added `UnsafeSliceDataAliasing`. The r43g row read 547/572/574 and was already one behind when it was written, so **measure, don't decrement**: `Get-ChildItem -Recurse -Filter *.go` on unique directories is the transpiled-package count, `check-solution-integrity.ps1` prints the registered count) | Go + C# | Per-feature Go↔C# equivalence (arrays, channels, defer, generics, interfaces…). |
...
- **Budget each command against its MEASURED baseline — the old flat "~3 min" cap is no longer right for
  ...
  **not** re-baseline these rows from a laptop run, and size timeouts from the top of the range.
  ...
  | `run-validated-sweep.ps1` (full roster) | **~46–53 min solo (3,138s measured 2026-08-07 at 109 packages / 13,611 verdicts; the roster is 110 / 13,628 since r47a; ~90+ min under two concurrent lane loads — both r47 attempts were killed externally before finishing, so no clean loaded figure exists)** | run it BACKGROUNDED from the COORDINATOR session only — ⚠ a LANE parking a detached sweep and ending its turn gets it KILLED (the lane's process tree is reaped; happened twice on 2026-08-08 at 106/110 and 98/110, log ends between packages with no summary — recovery: re-run `roster − logged` inline and check the verdict arithmetic closes) | ~29 s for a typical package; use `-Filter` for anything but a final gate. ⚠ Four packages carry per-package deadline FLOORS in the script's `$longTimeouts` (`hash/maphash` 30m, `index/suffixarray` 60m, `crypto/dsa` 30m, `archive/zip` 30m) and every one is **i9-calibrated** — `crypto/dsa`'s banked `TestParameterGeneration` alone is 1,156.8 s *inside* its 30m, and `archive/zip`'s Debug suite is 391s on the i9 vs 774s here — so this machine class can blow a floor and report `FAIL … package timeout after 00:30:00` on a package that is merely slow (hash/maphash and crypto/dsa both did, 2026-08-10). Since 2026-08-10 the table is a **floor, not an override**: a LARGER `-TestTimeout` raises it (a smaller one still loses, since under-budgeting these four is the false red the table exists to prevent), so the remedy on a slow box is `-TestTimeout 60m` — not editing the table, whose values are the floor a fast box needs. Measured here solo: `-Filter maphash -TestTimeout 60m` → **22/22 in 2,406 s (40.1 min)**, i.e. this machine needs ~1.3x the 30m floor for that one row |
