...
- **Budget each command against its MEASURED baseline — the old flat "~3 min" cap is no longer right for
  ...
  **not** re-baseline these rows from a laptop run, and size timeouts from the top of the range:
  ...
  | `check-no-regression.ps1` (full) | **~350–475s (6–8 min; 463s measured 2026-08-07 at **574 packages** with a sibling lane active; re-measured 2026-08-05 at 570; 358s and 366s on 2026-08-04 at 569; top of range is concurrent-lane load)** | 700s | transpile-only, no compile/run; re-transpiles unconditionally |
