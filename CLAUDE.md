...
| **Behavioral tests** | `src/Tests/Behavioral/` (544 test projects = 569 transpiled Go packages incl. 25 nested sub-libraries; 571 registered `.csproj` incl. the `BehavioralTests`/`BehavioralRunner` tooling — re-measured r40, 2026-08-04) | Go + C# | Per-feature Go↔C# equivalence (arrays, channels, defer, generics, interfaces…). |
...
- **Behavioral tests** (`src/Tests/Behavioral/`): each test references `golib` + the `go2cs-gen` analyzer;
  ...
  `reflect`). Since 2026-08-01 those references bind the **converted** packages, so the suite's 514
...
- **FALSE-GREEN route #3 — NESTED sub-library packages were never enumerated (fixed 2026-08-02).** All
  ...
  **recursively, DEEPEST-FIRST** (`GoPackageDirs` in `BehavioralRunner`/`BehavioralTestBase`; a recursive
  ...
  `UpToDate` in both runners considers the nested packages too. Enumeration is now **569 packages**
  (544 top-level + 25 nested; it was 543 = 521 + 22 when this note was written), not 521. Note goldens remain top-level-only: nested packages have no
...
- **Budget each command against its MEASURED baseline — the old flat "~3 min" cap is no longer right for
  ...
  |---|---|---|---|
  | `run-behavioral.ps1` (full, 4 phases) | **~370–1575s (6–26 min; 626s measured 2026-08-04 SOLO at 544 projects, 533s on 2026-08-02 at 528, 1575s the same day with THREE sibling worktrees running pipelines)** | 2100s | 544/544 Transpile+Compile+Target; 514 Output-compared, 30 skipped (no `package main`); the top of the range is concurrent-lane load — budget for it. ⚠ At that load the **Go toolchain itself** can crash building one project (`panic: … compress/flate.(*huff…` inside `go build`) and the runner reports it as a Go build failure; re-run that one project filtered before believing it |
  | `check-no-regression.ps1` (full) | **~350–475s (6–8 min; re-measured 2026-08-04 at **569 packages** — 358s and 366s across two runs — so the corpus has grown 26 packages since 2026-08-02 at no measurable cost; top of range is concurrent-lane load)** | 700s | transpile-only, no compile/run; re-transpiles unconditionally |
  ...
  | full `go2cs-stdlib.slnx` build | **~92–158s** warm (304 projects; 158s measured r40 with `-p:UseSharedCompilation=false`, the isolation flag a lane uses instead of `build-server shutdown`) | 600s | cold restore adds a few minutes |
...
- **One tree since 2026-08-01:** the stub baseline retired and the converted standard library moved into
  ...
  packages — **544/544** transpile+compile+golden, **514/514** stdout comparisons vs `go run`
  (r40, 2026-08-04, against the rebanked corpus). All rewrite machinery is gone: one path scheme,
