...
| **Behavioral tests** | `src/Tests/Behavioral/` (518 transpiled test projects, 541 registered `.csproj` incl. sub-libraries, + `BehavioralTests` runner) | Go + C# | Per-feature Go↔C# equivalence (arrays, channels, defer, generics, interfaces…). |
...
- **Budget each command against its MEASURED baseline — the old flat "~3 min" cap is no longer right for
  the full runs (re-measured 2026-08-01, corpus at 518 transpiled / 541 registered `.csproj`).** The
  corpus keeps growing (371 → 457 → 518 projects), and both full instruments
