...
| **Behavioral tests** | `src/tests/Behavioral/` (579 top-level test projects = 606 transpiled Go packages incl. 27 nested sub-libraries; 608 registered `.csproj` incl. the `BehavioralTests`/`BehavioralRunner` tooling — re-measured 2026-08-16 after the consolidation-week guards landed. The prior row read 549/574/576 (r44b, 2026-08-07) and had drifted ~30 behind, so **measure, don't decrement**: `Get-ChildItem -Recurse -Filter *.go` on unique directories is the transpiled-package count, `check-solution-integrity.ps1` prints the registered count) | Go + C# | Per-feature Go↔C# equivalence (arrays, channels, defer, generics, interfaces…). |
