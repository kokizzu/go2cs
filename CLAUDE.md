...
- **Budget each command against its MEASURED baseline — the old flat "~3 min" cap is no longer right for
  ...
  **not** re-baseline these rows from a laptop run, and size timeouts from the top of the range:
  ...
  `--build-timeout`/`GO2CS_BUILD_TIMEOUT` (batch build, **300**), `--build-one-timeout`/
  `GO2CS_BUILD_ONE_TIMEOUT` (per-project build, shared-dep pre-build, `go build`, **180**),
  `--transpile-timeout`/`GO2CS_TRANSPILE_TIMEOUT` (**60**), `--run-timeout`/`GO2CS_RUN_TIMEOUT`
  (one program run in the Output phase, **30**). The defaults are unchanged, so this fast-desktop lane
  keeps its existing fail-fast behavior — the overrides exist for machines the table above is *not*
  baselined on.
  **The slow-machine row this table was missing (measured 2026-08-10, i7-5820K 6C/12T, ~3x slower than
  ...
  cold** (573 projects, 0 errors), ~5x the batch budget; a single cold filtered project measured 163 s.
  On such a machine set `GO2CS_BUILD_TIMEOUT=1800` and `GO2CS_BUILD_ONE_TIMEOUT=600` once per shell, or
  pre-warm the tree with a full solution build before the run.
